defmodule Main do
  ...
  defp solve(n) do
    xy = for _i <- 1..n, do: Acl.Input.get_n_list()
    result = Enum.reduce(xy, [0, 0], fn x, acc ->
      [Enum.at(x, 0) + Enum.at(acc, 0), Enum.at(x, 1) + Enum.at(acc, 1) ]
    end)

    cond do
      Enum.at(result, 0) == Enum.at(result, 1) -> "Draw"
      Enum.at(result, 0) > Enum.at(result, 1) -> "Takahashi"
      true -> "Aoki"
    end
  end
end
